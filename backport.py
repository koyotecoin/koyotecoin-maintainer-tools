...
# Other configuration
SRCREPO = os.getenv('SRCREPO', '../koyotecoin')
...
for t in to_backport:
    ...
    for (cid, message) in reversed(commits):
        print('{a.hsh}#   {a.head2}{}{a.reset}'.format(
            cid + ' ' + message, a=Attr))
        ...
        if execute:
            if subprocess.call([GIT, 'cherry-pick', commit.hexsha]):
                # fail - drop to shell
                print(
                    'Dropping to shell - run git cherry-pick --continue after fixing issues, or exit and choose abort/skip')
                # Show pull number on Debian default prompt
                if os.path.isfile('/etc/debian_version'):
                    os.putenv('debian_chroot', t[0])
                subprocess.call([BASH, '-i'])
                reply = ask_prompt(
                    "Type 'c' to continue, 'a' to abort, 's' to skip pull.")
                if reply == 'c':
                    pass
                elif reply == 'a':
                    exit(1)
            ...
            subprocess.check_call(
                [GIT, 'commit', '--amend', '--gpg-sign', '-q', '-m', cmsg])
        else:
